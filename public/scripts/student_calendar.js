...
{
    return fetch('student-calendar')
    .then(onResponse)
    .then(onJSON)
    .catch(onError);
}
function updateStudentCalendar() {
    getStudentCalendar().then(function(data) {
        const contentDiv = document.querySelector('#student-calendar-block'); //primo da modificare
        removeAllChildren(contentDiv);
        const table = document.createElement('table');
        table.setAttribute('id', 'student-calendar-table'); //secondo parametro da cambiare
        ...
        for (let i = 1; i < (allRows.length); i++)
        {
            ...
            for (let j = 1; j < (daysCols.length); j++)
            {
                for (let elem of data)
                {
                    ...
                    {
                        const subjectText = document.createTextNode(elem['nome_disciplina']);
